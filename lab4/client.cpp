...
const unsigned int SHM_SIZE = 8192;
...
int main (int argc, char *argv[]) {
  ...
  try {
    int* keyPtr = (int*)shmKey.attach(SHM_KEY_KEY);
    semSess.attach(SEM_SESS_KEY);
  ...
  try {
    ...
    while(true) {
      ...
      if(size < 0) {
        ...
        return -1;
      } else if (size + sizeof(int) < SHM_SIZE) {
        ptrInt[0] = size;
        semServ.op(1);
        break;
      } else {
